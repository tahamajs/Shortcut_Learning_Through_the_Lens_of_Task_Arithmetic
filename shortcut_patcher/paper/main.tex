...
\textbf{Training defaults:} AdamW, warmup+cosine schedule, snapshots every 100 steps, and robust ERM-vs-GroupDRO comparison with group-balanced sampling on shortcut benchmarks.
...
\begin{itemize}[leftmargin=*]
  ...
  \item Best-checkpoint selection by validation worst-group accuracy
  \item Robustness gap
  \item Toxicity and perplexity for GPT-2
  \item Compute budget and wall-clock
\end{itemize}

\subsection{Robust optimization details}
For datasets with group metadata (e.g., Waterbirds), we train with two objectives:
\begin{itemize}[leftmargin=*]
  \item \textbf{ERM:} standard mean cross-entropy.
  \item \textbf{GroupDRO:} optimize weighted worst-group loss with multiplicative updates over group weights.
\end{itemize}
At each validation interval, we log both overall and worst-group metrics and checkpoint the model maximizing validation worst-group accuracy.
...
\section{Conclusion}
This artifact provides a full, reproducible implementation for identifying shortcut directions and mitigating them with robust optimization + task arithmetic. The code is intentionally modular so real CLIP/GPT-2 pipelines can replace synthetic loaders with minimal changes.
