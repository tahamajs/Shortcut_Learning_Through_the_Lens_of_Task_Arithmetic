...
def _infer_num_classes_from_state_dict(state_dict: Dict[str, torch.Tensor]) -> int | None:
    for key in ("fc.weight", "net.4.weight", "classifier.weight"):
        if key in state_dict and torch.is_tensor(state_dict[key]) and state_dict[key].ndim == 2:
...
def load_checkpoint_into_model(model_name: str, ckpt_path: Path) -> torch.nn.Module:
    ...
    inferred_classes = _infer_num_classes_from_state_dict(state_dict)
    if inferred_classes is None:
        model = load_pretrained(model_name)
    else:
        model = load_pretrained(model_name, n_classes=inferred_classes)
